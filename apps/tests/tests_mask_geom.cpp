...
//
// ⚠ **Split again 2026-08-07 at 1,173 lines**, over `CLAUDE.md`'s 1,000-line
// ceiling. The cut is at a seam rather than a line number, and the seam is
// decision #138: the checks on the **pull-back** — the one 3 × 3 the renderer
// actually evaluates each pixel through — moved to `tests_mask_pullback.cpp`.
//
// ⚠⚠ **What is left grades the forward transport, and three of the functions it
// grades have had no product caller since #138** (#184): `radiusToFrame`,
// `lengthToFrame` and `lengthAlong` push a mask into frame coordinates through
// the map's derivative at a single point, which is what the kernel stopped
// doing. Read these as checks on **live** Jacobian maths observed *through* a
// lens that is not shipped — `testPerspectiveMaskExtent`'s 6b is the only thing
// in the tree that catches `unperspective`'s conjugation (#178) — and not as
// coverage of what a photograph renders.
