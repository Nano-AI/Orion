...
| `balance-survives-a-reopen.txt` | Passes — the grading Balance through slider, `DevelopState`, sidecar and decode, decision #102. ⚠ It **pushes the shadow wheel first, on purpose**: Balance slides the zones the wheels act on, and with every wheel centred it is an exact identity by construction, so a render check on a neutral frame would be green for a control wired to nothing. Dropping `gradeBalance:` from the memberwise initializer in `Engine.state` — which compiles silently, and is how film grain reached the sidecar as 0 — turns this file red and nothing else in the repository |
