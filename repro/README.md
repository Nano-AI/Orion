...
| `perspective-carries-the-mask.txt` | Passes — the three fields through the whole loop, the zoom leaving no hole, and a mask still on its subject with the correction up. ⚠ Its keystone half moved from 0.28 to **0.34** on 2026-08-01, but that is not what makes it catch anything: 0.34 × 0.22 passed before the fix too. Its **aspect** section is the one that fails when a mask's extent goes back to one isotropic scale — 4 of 84 clear cells at 0.1461 luma, because a squeeze has no curvature to hide behind (decision #102) |
...
| `balance-survives-a-reopen.txt` | Passes — the grading Balance through slider, `DevelopState`, sidecar and decode, decision #104. ⚠ It **pushes the shadow wheel first, on purpose**: Balance slides the zones the wheels act on, and with every wheel centred it is an exact identity by construction, so a render check on a neutral frame would be green for a control wired to nothing. Dropping `gradeBalance:` from the memberwise initializer in `Engine.state` — which compiles silently, and is how film grain reached the sidecar as 0 — turns this file red and nothing else in the repository |
