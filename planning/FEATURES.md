...
**M6 added 2026-08-10 (#199)**: the committed batch from the Lightroom audit, by developer instruction — stories and costs in `ROADMAP.md`. The audit's bigger items are recorded in §15 and are **not** committed; nothing there gets code before a decision row settles it.
...
| DNG | M2 | |
| Linear DNG (demosaiced) read | ✅ 2026-08-24 | The develop graph grows a one-node linear head; WB/matrix path identical from there on. Built for the HDR merge, works for any LinearRaw file |
| **HDR merge (2+ frames → linear DNG)** | ✅ 2026-08-24 | Handheld: ORB+RANSAC+ECC alignment (OpenCV, #192), SNR-weighted radiance merge, reference-consistency deghost. Output lands beside the sources and is a normal library photo. #189–#192, `research/hdr-merge.md` |
...
| White balance (temp/tint, eyedropper, presets) | M1 | ⚠ **Corrected 2026-08-10, #198: only temp/tint shipped.** The eyedropper and the presets this row has claimed since scope lock exist nowhere in the tree — `DevelopPanels+Light.swift` is two sliders. Queued **M6, S6.1**; the click-a-neutral inversion needs a `research/` entry before code |
| B&W conversion + per-band gray mix | M6 | S6.2 (#199) — the largest Lightroom parity hole: nothing in the tree maps color to gray per band. LUTs cover looks, not the mix control |
...
| Defringe (manual purple/green) | M6 | S6.7 (#199) — lensfun's CA correction ships (§7); manual fringe suppression does not, and needs its own `research/` citation |
...
| Copy/paste settings, sync across selection | M4 | ✅ **Built 2026-07-30**. Sync patches sidecars at the JSON-key level without decoding them, so an unedited photo keeps its as-shot white balance. Confirmed, with the group names in the question. ⚠ Applies to every photo in view — no multi-selection yet. Selection scoping queued **M6, S6.10** (#199) |
...
| Export presets + JPEG XL/AVIF probe | M6 | S6.11 (#199) — named export settings; verify ImageIO support before promising either format |
...
| Before/after, split view | M2 | Split-with-divider ships (`CompareOverlay`); the Y-style side-by-side does not — queued **M6, S6.6** (#199) |
| Side-by-side compare | M6 | S6.6 (#199) |
| Busy state on photo open | M6 | S6.3 (#199) — #151: the canvas holds the *previous* photograph for the 210.9 ms of a cold open, and nothing anywhere marks it stale |
| Slider color scales | M6 | S6.4 (#199) — temp blue→yellow, tint green→magenta, mixer tracks tinted per band. `AnalogTrack` is flat + accent today |
| Interactive histogram | M6 | S6.5 (#199) — clipping overlay on the image, drag a region to move the matching slider |
| Soft proofing | M6 | S6.8 (#199) — lcms2 + OCIO already in the stack; UI plus a proof transform |
...
## 15. Recorded, not committed — needs a decision before any code (2026-08-10, #199)
