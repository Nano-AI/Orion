...
| **The 1000-line rule is broken six ways, and only two of them are in `app/`.** ⚠ **Recounted by sweep 2026-08-02** — `git ls-files '*.cpp' '*.h' '*.swift' '*.slang' \| xargs wc -l`, at `9d9158d`, not by adjusting the previous numbers: `apps/tests/tests_effects.cpp` **1,716**, `app/Scenario.swift` **1,615**, `app/DevelopPanels.swift` **1,366**, `apps/tests/tests_brush.cpp` **1,142**, `apps/tests/tests_perspective.cpp` **1,110**, `apps/tests/tests_grade.cpp` **1,029**. `engine/src/pipe/ShaderParams.h` at **905** is the next below. ⚠ **The four test files are the finding, and they have never been in this row.** The recount written the same morning said "the next file below the survivors is `ShaderParams.h` at 905", which was wrong by four files — and **the largest file in the tree is now a test**, larger than any of the three the fifth wave was chartered against. It also carried `Scenario.swift` at 1,596, which `9d9158d` had already made 1,615. ⚠ **Recount the sweep before editing this row; do not adjust the numbers in place** — it once carried four contradictory copies of itself, and has now been wrong twice running. `OrionApp.swift` left the row at #121 (**1,557 → six files, largest 373**); `Scenario.swift` and `DevelopPanels.swift` were in flight beside it and are counted here as of `9d9158d` | whole tree |
