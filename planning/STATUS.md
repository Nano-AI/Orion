...
**Previously:** 2026-08-14 (**#200**: the matte's size had two derivations,
one truncating and one rounding, so every subject selection on a 7968 x 5320
sensor was refused — the producer asks the engine now. **#201**: the canvas had
no mip chain, so fitting a 42 Mpx frame to a window decimated it and showed
**2.5x** the noise of a proper average; display only, the export is unchanged.
**#202**: AgX's black latitude cut from the reference 10 stops under middle gray
to darktable filmic's 8, after the darkest patch of a photograph measured 1.63x
brighter than macOS ImageIO's decode of the same raw. ⚠ check-modes and
check-screens cannot run at all — `samples/*.ARW` point at three folders that no
longer exist.)

**Before that:** 2026-08-10 (**#198**: the armed-click flags are one
`CanvasTool` enum and one `ToolButton` template — the mask color picker used to
survive a tab switch and eat the next click. **#199**: the Lightroom audit's
must-do batch is committed as **M6**, S6.1–S6.11 costed in `ROADMAP.md`; its
maybes are FEATURES §15, recorded and explicitly *not* scope. ⚠ FEATURES had
overclaimed twice — WB eyedropper (#198) and nothing else survived checking:
colour range and sky masks both ship. Licence: **Apache-2.0, #188**)
...
**The queue is M6 (#199), in story order** — S6.1 WB eyedropper (research entry
first) → S6.2 B&W mix → S6.3 busy state → S6.4 slider color scales → S6.5
interactive histogram → S6.6 side-by-side → S6.7 defringe → S6.8 soft proofing
→ S6.9 FSEvents watch → S6.10 multi-selection → S6.11 export presets. Costs in
`ROADMAP.md` §M6. ⚠ **#162 outranks the whole queue and is blocked on the
developer's choice** (tiling / lower precision / refuse early). FEATURES §15
holds the recorded-not-committed list; nothing there is queued.
...
- ~~⚠⚠ **The licence.**~~ ✅ **Resolved 2026-08-07, #188 — Apache-2.0**, chosen
  by the developer; canonical text verified by hash, `NOTICE` carries the
  third-party obligations. A licence is still not a freedom-to-operate
  assessment (#174).
...
## Session `2026-08-14` — three display-path faults the developer's new camera exposed, #200–#202
...
## Session `2026-08-10b` — the audit becomes scope: M6 committed, the maybes fenced, #199
...
Y-style side-by-side is absent. Recorded in #199 so the first draft is not
...
## Session `2026-08-10` — the armed-click flags become one enum and one template, #198
