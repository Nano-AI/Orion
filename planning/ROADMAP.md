...
| ⚠ **Dehaze's drag cost has roughly doubled — confirmed 2026-08-01, not yet root-caused** | Normalised against exposure *in the same process*, so machine load cannot explain it: dehaze/exposure was **7.2×** when `repro/slider-drag-cost.txt` was written and is **11.8–13.5×** now. `orion-bench` agrees independently — dehaze does twice the work of clarity in 40% of the nodes. **Re-measured 2026-08-01 on a quiet machine, paired, twice:** exposure 10.3/11.3 ms a tick against 9.3 recorded, clarity 64.1/63.5 against 60.5 — both within noise — and **dehaze 125.9/124.9 against 67.3**. So it is dehaze alone, it is reproducible, and it is not load. ⚠ **Narrowed but not explained:** `orion-bench`'s own per-node profile for the same control totals **73.62 ms across 19 nodes**, so roughly **50 ms of a 125 ms tick happens outside node dispatch**. `estimateAirlight` is the obvious suspect — it downloads a texture and forces a second `pipeline_.render()` — but `render()`'s comment says it cannot fire on the interaction path, and `airlightValid_` is cleared only by a white-balance change. Either that comment is wrong or the time is somewhere else. **Next step is a git bisect**, which needs a rebuild per step and is its own session; guessing further is how this file's two wrong performance claims were made |
