...
| ✅ **The tick, attributed** | **Done 2026-08-03, decision #149.** ⚠ The reported *"sliders slow"* is answered, and not by making a filter cheaper: **clarity costs 4.2 ms to drag and 56.4 ms to release**. Measured both ways in one run on `_PIC8220`, 40 ticks each — exposure 9.4 → 4.5 ms (2.1×), clarity 56.4 → 4.2 ms (**13.4×**), dehaze 79.0 → 6.5 ms (**12.2×**). ⚠ `repro/slider-drag-cost.txt` had measured only the unarmed path since it was written, so it kept reporting 17 fps for a gesture that runs at 235; its own text said degrade-then-refine *"was never built"* long after it was. Both paths are measured there now, and both must stay — the full-resolution number is still what a commit, an export and a keyboard nudge pay |
| ✅ **Cold open** | **Measured 2026-08-03, decision #151.** **210.9 ms**, eight repeat opens of `_PIC8220` on a quiet machine — not the ~108–128 ms this row's own *36 + 72–92* implies, which is the fifth stale figure this audit has found. ⚠ **What the photographer sees is the previous photograph.** `isLoaded` is set once at `Engine+Document.swift:86` and never cleared, so the canvas holds the old render until the new one lands, and there is **no busy state anywhere in the application** to mark it stale. First open of a session is honest — the empty state shows. ⚠ Whether holding the last frame beats a flash of black is a design question, deliberately not settled here; it looks worst in the flat-frame case, where a correct picture lingers over a wrong one |
...
| ✅ **Scroll, zoom, pan, filmstrip** | **Answered 2026-08-03, decision #154 — by reading, and it says so.** Zoom and pan apply a transform to the **already-rendered** texture in the canvas blit (`ImageCanvas.swift:348–353`), with a nearest sampler past 4×; neither re-runs the graph, so the 55 ms render and the 7,186 MiB of intermediates are irrelevant to them. The filmstrip is a `LazyHStack` over `library.visible`. ⚠ **No number is quoted on purpose**: driving a scroll or a magnify needs a gesture, and #110.3 established this harness cannot drive one — the same limit that makes `check-gestures.py` a grep. Closing it properly wants a verb that drives `Viewport.zoomBy` and the filmstrip offset directly, below the gesture and above the render |
| ✅ **Memory** | **Measured 2026-08-03, decision #152.** **7,186 MiB of intermediates against an 18,186 MiB working set here — 40%.** ⚠ **An 8 GB Mac has about 6,144 MiB**, so a 24 MP frame is over by a gigabyte before anything else on the system counts, and the `CLAUDE.md` floor of macOS 14 admits the base M1 and M2 Air. **Nothing in the engine checks**: `recommendedMaxWorkingSetSize` is read, carried and exposed, and never compared against what the graph allocates — the bench printed both numbers with no relation between them. It prints the ratio now and warns outright. ⚠ **Not fixed, deliberately**: tiling, lower intermediate precision, or refusing early with a usable sentence are three different stories, and picking one at the end of a session is how the wrong one gets built |
...
|---|---|---|
| ⚠ **Half of this was already done — corrected 2026-08-04 (#166).** | The row said rows are *never* collected and that the prune only ever cleans a folder you are looking at. **A folder that no longer exists is collected at startup**: `PhotoIndex.collectMissingFolders()`, called from `init` at `PhotoIndex.swift:213`, deletes from `photos` and `thumbnails` by `dir` for every directory that has gone — and it is tested, with the named mutation *drop the `collectMissingFolders()` call*. **What remains is narrower and real**: a folder that still exists on disk and is simply never opened again keeps its rows and its thumbnails forever, because nothing walks the index looking for stale-but-present directories. That is a budget question rather than a correctness one, and it wants the thumbnail readout it has no surface for |
| ⚠ **Wrong in two of its three claims — corrected 2026-08-04 (#167).** | It read *"a constant with no way to see it or clear it. 512 MB, no readout, no button"*. **It is not a constant**: `PhotoIndex.init(at:budgetBytes:)` takes it, defaulting to `512 << 20`, and the eviction tests already drive it at **4096 bytes**. **There is a readout**: `heldBytes` (`PhotoIndex.swift:536`), lock-guarded, and pinned by an existing check asserting *two thumbnails are 3000 bytes*. **Only the button is missing**, and a button needs the preferences surface the app still does not have — the same surface the stale-but-present folder collection needs (#166), which is why those two are one story. ⚠ Nearly shipped a **second accessor for the same number** while checking this; `heldBytes` was found only by reading the test that uses it |
...
| ✅ **A `SQLITE_BUSY` from a second Orion is tested.** | **Row was stale, corrected 2026-08-04 (#164).** `testALockedDatabaseIsNotDiscarded` has existed and been registered at `ViewportTests.swift:135`; it takes an exclusive transaction on a second connection and asserts `index.available` survives — a lock is somebody else working, not damage. ⚠ **The row was wrong about the method too**: it asked for a second *process*, and SQLite locks the file, so a second **connection** contends identically. ⚠ And the test's own header records that **its first version could not fail** — it asserted the row survived into a new `PhotoIndex`, assuming condemning deletes the file; `discardable` is consulted only in `init`, so a busy write leaves the file alone. The mutation (`guard code != SQLITE_OK`) passed until it was rewritten |
...
## M6 — Parity & polish (added 2026-08-10, #199)
...
| Story | Cost | Notes |
|---|---|---|
| S6.1 WB eyedropper + presets | ~1–2 sessions | The `research/` entry for the click-a-neutral temp/tint inversion comes first (#198 found FEATURES had claimed this shipped since scope lock). Lands as a fourth `CanvasTool` case, one `ToolButton` line |
